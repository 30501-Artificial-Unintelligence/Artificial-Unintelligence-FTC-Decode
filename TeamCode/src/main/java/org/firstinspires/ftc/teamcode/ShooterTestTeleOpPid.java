...
@TeleOp(name = "Shooter Test pid", group = "Test")
public class ShooterTestTeleOpPid extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        telemetry.addLine("START: cycle PID param (kP/kI/kD)");
        telemetry.addLine("SQUARE (X): - selected PID, CIRCLE (B): + selected PID");
        telemetry.update();
        ...
        while (opModeIsActive()) {
            ...
            if (start1 && !prevStart1) {
                pidParamIndex = (pidParamIndex + 1) % 3; // 0 -> 1 -> 2 -> 0
            }
            prevStart1 = start1;
            ...
            // PID values + which one is selected
            String selectedParamName =
                    (pidParamIndex == 0) ? "kP" :
                            (pidParamIndex == 1) ? "kI" : "kD";

            telemetry.addData("PID Selected", selectedParamName);
