...
public class ThreeBallPathAuto extends LinearOpMode {
    ...
    private void moveToFirstShootingPosition() {
        ...
        drive.driveForwardMm(this,1200,0.5);
    ...
    private void shootMagazineFastNoPattern() {
        ...
        long spinupEnd = System.currentTimeMillis() + 1500; // ~0.8s, tune as needed
        ...
        // ----- 2) Start eject sequence and let it run to completion -----
        boolean startedEject = false;

        while (opModeIsActive()) {
            // On the first pass, give spindexer a Y rising-edge to start ejecting
            boolean yEdge = !startedEject;

            // tagOverride = 0 → "fastest possible" order
            spindexer.update(telemetry, loader, yEdge, 0);
            loader.updateLoader();

            // Keep shooter running (no toggles, just "wantOn = true")
            shooter.update(true, false, false, fieldPos);
            ...
            // Done when:
            //  - spindexer is no longer in eject sequence, AND
            //  - it thinks there are no balls left
        ...
        long holdEnd = System.currentTimeMillis() + 300; // extra 0.3s if you want
        ...
        // Finally turn shooter off
        shooter.update(false, false, false, fieldPos);
        shooter.stop();    // just to be extra sure motor power = 0
    }
